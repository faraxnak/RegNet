...
#include "caffe/layers/dense_register_data_layer.hpp"
...
DenseRegisterDataLayer<Dtype>::~DenseRegisterDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void DenseRegisterDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  while (infile >> filename >> floatImage_fileName >> label_filename) {
    std::vector<string> row;
    row.push_back(filename);
    row.push_back(floatImage_fileName);
    row.push_back(label_filename);
  ...
  cv::Mat cv_lab = ReadImageToCVMat(root_folder + addresses[lines_id_][2],
...
template <typename Dtype>
void DenseRegisterDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void DenseRegisterDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    cv::Mat cv_lab = ReadImageToCVMat(root_folder + addresses[lines_id_][2],
        new_height, new_width, false, true);
    CHECK(cv_lab.data) << "Could not load " << addresses[lines_id_][2];
...
INSTANTIATE_CLASS(DenseRegisterDataLayer);
